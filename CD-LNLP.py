...
if __name__ == '__main__':
    ...
    for i in range(0, len(one_list), split):
        ...
        input_data = np.random.randn(100, 64).astype(np.float32)
        labels = np.random.randint(0, 2, size=(100,)).astype(np.int32)
        input_data_ms = Tensor(input_data)
        labels_ms = Tensor(labels)
        epoch = 10
        for i in range(epoch):
            # 前向传播
            output = net(input_data_ms)
            # 计算损失
            loss = loss_fn(output, labels_ms)
            # 反向传播并更新参数
        ...
        accuracy_list = []
        F1_list = []
        arr = mnp.random.randn(100, 100).astype(np.float32)
        arr_mean = mnp.mean(arr, axis=1)
        arr_std = mnp.std(arr, axis=1)
        ...
        for num in top_list:
            ...
            for tmp in tmp_list:
                with open(
                        './Data/large_circRNA_cancer/circRNA_cancer_fold{}/CD-LNLP_fold{}_top{}.txt'.format(fold, fold, num),
                        'a') as fs:
